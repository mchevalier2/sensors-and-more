...
class TestSensor(unittest.TestCase):
    ...
    def test_weekdays_open(self):
        for test_day in range(5, 10):
            with self.subTest(i=test_day):
                ...
                visit_count = visit_sensor.get_visit_counts(datetime(2024, 8, test_day, 12, 0, 0))
    ...
    def test_opening_hours(self):
        for test_hour in range(8, 18):
            with self.subTest(i=test_hour):
                ...
                visit_count = visit_sensor.get_visit_counts(datetime(2024, 8, 2, test_hour, 0, 0))
                self.assertFalse(visit_count == 0)
    ...
    def test_anomalies(self):
        visit_sensor = Sensor(0, 1000, 150, start_time=datetime(2021, 9, 15, 10, 3, 30), p_fail=0.05, p_anom=0.2)
